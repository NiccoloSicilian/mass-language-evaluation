...
@torch.no_grad()
def avg_layers(svd_dict, device="cuda"):
    """
    Computes the arithmetic mean of task vectors reconstructed from their
    SVD-compressed representations.

    Args:
        svd_dict: {dataset -> {layer_name -> {"u","s","v"} or {"dim1"}}}.
        device:   Target device for computation.

    Returns:
        dict {layer_name -> averaged task-vector tensor}.
    """
    datasets = list(svd_dict.keys())
    ...
    for layer_name in tqdm(reference_keys, desc="Averaging SVD Task Vectors"):
        is_matrix_layer = "u" in svd_dict[datasets[0]][layer_name]
        layer_sum = None

        for dataset in datasets:
            if is_matrix_layer:
                entry = svd_dict[dataset][layer_name]
                u = entry["u"].to(device)
                s = entry["s"].to(device)
                v = entry["v"].to(device)
                delta = u @ torch.diag_embed(s) @ v
            ...
            layer_sum = delta if layer_sum is None else layer_sum + delta
